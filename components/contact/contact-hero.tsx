...
export default function ContactHero() {
  ...
  return (
    <div className="relative pb-32 pt-28 bg-background overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 opacity-20">
          <div className="h-full w-full grid grid-cols-12 gap-4">
            {Array(12)
              .fill(0)
              .map((_, i) => (
                <div key={i} className="h-full border-r border-primary/30"></div>
              ))}
          </div>
          <div className="h-full w-full grid grid-rows-12 gap-4">
            {Array(12)
              .fill(0)
              .map((_, i) => (
                <div key={i} className="w-full border-b border-primary/30"></div>
              ))}
          </div>
        </div>
        ...
        />
        <motion.div
          className="absolute bottom-20 left-[10%] w-72 h-72 rounded-full bg-accent/20 blur-3xl"
          animate={{
            scale: [1, 1.3, 1],
            opacity: [0.3, 0.6, 0.3],
          }}
        ...
        />
        <motion.div
          className="absolute top-40 left-[30%] w-48 h-48 rounded-full bg-primary/20 blur-3xl"
          animate={{
            scale: [1, 1.4, 1],
            opacity: [0.2, 0.4, 0.2],
          }}
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          >
            <h1 className="text-5xl md:text-6xl font-heading mb-6">
              <span className="text-foreground">Let's Solve Your</span> <br />
          ...
          <motion.p
            className="text-heading-md text-muted-foreground mb-10"
            initial={{ opacity: 0 }}
          ...
          >
            {[
              { value: "87%", label: "Increase in brand engagement" },
              { value: "3.2x", label: "Higher conversion rates" },
              { value: "92%", label: "Client satisfaction" },
            ].map((stat, index) => (
              <motion.div
                key={index}
                className="bg-background/80 backdrop-blur-sm border border-primary/30 p-4 rounded-lg text-center"
                whileHover={{
                  scale: 1.05,
              ...
              >
                ...
                <p className="text-body-sm text-foreground">{stat.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
